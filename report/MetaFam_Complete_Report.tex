...
\begin{enumerate}
    ...
    \item \textbf{Task 5 (Feature-Enhanced CompGCN):} Implemented CompGCN with custom node features (gender, generation, centrality, community ID) and RotatE decoder. CompGCN-Custom achieved 0.677 MRR---best among GNN models (+53\% over RGCN) but below standard RotatE (0.791), revealing that MetaFam's disconnected topology limits feature-enhanced GNN approaches.
\end{enumerate}
...
\begin{itemize}
    ...
    \item Feature-enhanced CompGCN with cross-task knowledge integration and leakage-safe community features
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{generation_distribution.png}
...
\begin{enumerate}
    ...
    \item \textbf{Louvain:} Greedy modularity optimization (fast, scalable)
...
\textbf{Yes, perfectly.} Each detected community corresponds exactly to one of the 50 connected components (family trees). Number of Communities can be visualised using the gephi visualisation which shows 50 communities.
...
\textbf{Reason:} Without spouse relations, there are no inter-family connections that would create significant bridge nodes.
...
\begin{enumerate}
    \item At each step, with probability $(1-c)$, follows a random edge
    \item With probability $c$ (restart probability), returns to the source node
\end{enumerate}
...
\textbf{Implementation:} Used Personalized PageRank with restart probability $c = 0.15$.
...
\begin{equation}
    K(s,t) = \sum_{l=1}^{\infty} \beta^l \cdot |\text{paths of length } l \text{ from } s \text{ to } t|
\end{equation}
...
\textbf{Key Insight:} Katz is computationally expensive but excels at detecting close family even without direct edges---siblings share many indirect paths through parents; cousins share paths through grandparents.

\subsection{Proposed Relationship Strength Score}

We propose a combined \textbf{Relationship Strength Score} leveraging both proximity measures:

\begin{equation}
    S(u, v) = \alpha \cdot \text{RWR}(u,v) + \beta \cdot \text{Katz}(u,v)
\end{equation}
...
\subsection{Group B: Inverse Parent-Child and Gender Inverse Rules}

\begin{enumerate}
    \setcounter{enumi}{3}
    \item \textbf{Parent/Child Inverse:} Father(x,y) $\rightarrow$ Child(y,x)
    \item \textbf{Gender Inverse:} Sister(x,y) $\land$ isMale(y) $\rightarrow$ Brother(y,x)
\end{enumerate}

\subsection{Group C: Symmetric Sibling Relation}
...
\subsection{Group D: Complex First Cousin Once Removed Rules}

These rules define the ``first cousin once removed'' relationship with two directional variants:

\begin{enumerate}
    \setcounter{enumi}{6}
    \item \textbf{First Cousin Once Removed (Type A):} The child (entity A) of one's parent's cousin (entity B)---one generation below the first cousin.
    \item \textbf{First Cousin Once Removed (Type B):} One's parent's first cousin (entity A) is one generation above the querying person (entity B).
\end{enumerate}

\textbf{Note:} Type A represents the relationship from the perspective of the older generation looking down, while Type B represents the perspective from the younger generation looking up.
...
\begin{figure}[H]
    ...
    \caption{Rule confidence comparison. Transitive rules (1-3) and inverse rules (5-6) achieve perfect confidence, while the parent/child inverse (4) shows 83\% due to incomplete bidirectional edges. Complex cousin rules (7-8) have 0\% confidence due to missing relation types or wrong rules.}
    \label{fig:rule_conf}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{support_vs_success.png}
    \caption{Support vs. Success count for each rule. Rules with identical support and success achieve 100\% confidence. Rules 7 and 8 have support but zero success, indicating the conclusion relation type is absent from the dataset or relationship implied is incorrect.}
...
\textbf{Note:} A detailed technical report covering all implementation details, mathematical derivations, and extended analysis is available in \texttt{task4\_report.tex}.
...
\begin{itemize}
    \item \textbf{TransE:} Models relations as translations ($h + r \approx t$). Handles composition and inverse but not symmetric relations.
    \item \textbf{DistMult:} Bilinear diagonal model ($\langle h, r, t \rangle$). Handles symmetric relations but not anti-symmetric or inverse.
    \item \textbf{ComplEx:} Complex-valued DistMult with conjugation. Handles anti-symmetric and inverse relations via the conjugate mechanism.
    \item \textbf{RotatE:} Models relations as rotations in complex space ($h \circ r = t$). Theoretically handles all relation patterns including composition.
\end{itemize}
...
\subsection{GNN Approaches}

We implemented RGCN (Relational Graph Convolutional Network) as an encoder with two decoder combinations:
\begin{itemize}
    \item \textbf{RGCN + DistMult:} RGCN encodes nodes via message passing, DistMult scores triples
    \item \textbf{RGCN + RotatE:} RGCN produces complex-valued embeddings, RotatE decoder scores triples
\end{itemize}

\subsection{Evaluation Metrics}

\begin{itemize}
    \item \textbf{MRR:} Mean Reciprocal Rank of correct answers (1.0 = perfect)
    \item \textbf{Hits@1:} Fraction with correct answer ranked first
...
\begin{enumerate}
    \item \textbf{ComplEx Dominates:} Contrary to theoretical expectations that RotatE should excel, ComplEx achieved the best performance across all split types and metrics. This is explained by the \textbf{``Over-Regularization'' Hypothesis}:
    
    \begin{itemize}
        \item MetaFam is a \textbf{noise-free, synthetic dataset}---logical rules (e.g., sister is always female) are 100\% consistent
        \item Libraries like PyKEEN are tuned for \textbf{noisy, real-world data} (Freebase, Wikidata) and apply heavy regularization (L2, N3, Dropout)
        \item On clean data, \textbf{overfitting is actually beneficial}---our custom ComplEx essentially ``memorizes'' the perfect logic of the family tree without regularization holding it back
        \item PyKEEN ComplEx ``plays it safe'' and underperforms on this specific clean dataset
    \end{itemize}
    
    \item \textbf{RotatE Custom vs PyKEEN:} Our custom RotatE achieved only 0.2 MRR while PyKEEN RotatE achieved 0.76 MRR. This is explained by the \textbf{``Modulus Drift'' Problem}:
    
    \begin{itemize}
        \item In family trees, relations are \textbf{compositional}---\texttt{greatGrandsonOf} is essentially $r_{son} \circ r_{son} \circ r_{son}$
        \item RotatE requires the constraint $|r| = 1$ (unit magnitude), but our custom implementation lacks this constraint
        \item Without this constraint, if $|r_{son}| = 1.1$, then for a great-grandson (3 hops), magnitude becomes $1.1^3 \approx 1.33$
        \item If $|r_{son}| = 0.9$, magnitude becomes $0.9^3 \approx 0.72$
        \item This \textbf{``modulus drift''} causes embedding vectors to either vanish (shrink to 0) or explode, destroying the geometric structure needed for precise reasoning on deep queries
        \item PyKEEN's RotatE properly enforces $|r| = 1$, explaining its superior performance
    \end{itemize}
...
% ============================================================================
% PART V: FEATURE-ENHANCED LINK PREDICTION (TASK 5)
% ============================================================================
\newpage
\part{Feature-Enhanced Link Prediction with CompGCN}

\section{Introduction to Task 5}

Building on the link prediction results from Task 4, this task investigates whether \textbf{domain-specific node features} derived from earlier analyses can improve link prediction. We employ \textbf{Composition-Based Multi-Relational Graph Convolutional Networks (CompGCN)} \cite{vashishth2020compgcn}, a GNN architecture that jointly embeds entities and relations through learnable composition operators, enhanced with custom node features from Tasks 1--3.

\subsection{Objectives}

\begin{enumerate}
    \item Implement CompGCN with custom node features (gender, generation, centrality, community membership)
    \item Compare feature-enhanced CompGCN against standard RotatE with random initialization
    \item Analyze whether cross-task knowledge transfer improves link prediction on MetaFam
    \item Ensure data leakage prevention when using community features
\end{enumerate}

\subsection{Key Innovation}

Unlike the RGCN models in Task 4 which used randomly initialized entity embeddings, CompGCN-Custom replaces initial representations with a \textbf{feature projection layer} that maps engineered features to the embedding space. This constitutes the first model in our pipeline to integrate outputs from multiple earlier tasks.
...
\begin{table}[H]
    ...
    \label{tab:compgcn_features_complete}
    \begin{tabular}{llcc}
        \toprule
        \textbf{Feature} & \textbf{Source Task} & \textbf{Encoding} & \textbf{Dims} \\
        \midrule
...
Community detection is performed \emph{exclusively} on the training graph (edges from \texttt{train.txt}). Test edges are never used during Louvain community assignment, ensuring strict separation between feature engineering and evaluation. This is critical because community structure can encode edge existence information.
...
\begin{enumerate}
    \item \textbf{Feature Projection:} A custom \texttt{FeatureProjectionRepresentation} module projects sparse $F$-dimensional features to a dense 64-dimensional space:
    \[
    ...
    \item \textbf{CompGCN Message Passing (2 layers):} Composition-based aggregation over typed neighborhoods:
    \[
    ...
    \item \textbf{RotatE Decoder:} Scores triples via rotation in complex space: $f(h, r, t) = -\|\mathbf{h} \circ \mathbf{r} - \mathbf{t}\|$
\end{enumerate}

\subsection{Implementation Strategy}

We use a ``hot-swap'' approach with PyKEEN: build the standard \texttt{CombinedCompGCNRepresentations}, then replace the default entity representations with our custom projection layer. This preserves PyKEEN's training, evaluation, and early stopping infrastructure while enabling domain feature injection.
...
\begin{table}[H]
    \centering
    \caption{CompGCN Training Configuration}
    \begin{tabular}{ll}
        \toprule
...
\begin{table}[H]
    \centering
    \caption{CompGCN-Custom vs Standard RotatE (Transductive Split)}
    \label{tab:compgcn_head_to_head}
    \begin{tabular}{lcccc}
        \toprule
        ...
        \midrule
        $\Delta$ (Custom $-$ Standard) & \textcolor{red}{$-$0.114} & \textcolor{red}{$-$0.167} & \textcolor{red}{$-$0.081} & \textcolor{red}{$-$0.004} \\
        \bottomrule
    \end{tabular}
\end{table}

\textbf{Finding:} Contrary to the initial hypothesis, CompGCN-Custom underperforms standard RotatE across all metrics. The gap is large for precision (Hits@1: $-$16.7 pp) but nearly zero for broad recall (Hits@10: $-$0.4 pp).

\subsection{Ranking Across All Models}

\begin{table}[H]
    \centering
    \caption{CompGCN in Context: All Link Prediction Models (Transductive Split)}
    \begin{tabular}{clcc}
        \toprule
        \textbf{Rank} & \textbf{Model} & \textbf{MRR} & \textbf{Hits@10} \\
        \midrule
        1 & ComplEx (KGE) & \textbf{0.852} & \textbf{0.986} \\
        2 & Standard RotatE (KGE) & 0.791 & 0.972 \\
        3 & TransE (KGE) & 0.706 & 0.970 \\
        \rowcolor{blue!8}
        4 & \textbf{CompGCN-Custom (GNN+KGE)} & 0.677 & 0.968 \\
...
CompGCN-Custom is the \textbf{best GNN-based model} tested, outperforming RGCN\_RotatE by 53\% (MRR) while falling 14\% short of standard RotatE.

\section{Analysis and Insights}

\subsection{Why Custom Features Did Not Help}

\subsubsection{Feature Projection Bottleneck}
The projection compresses $\sim$58 sparse dimensions into 64 dense dimensions. Community one-hot encoding (50 dims, only 1 active) dominates the input but contributes minimal gradient signal per dimension. Meanwhile, standard RotatE learns 64 \emph{unconstrained} dimensions---a strictly more expressive parameterization.

\subsubsection{Community Feature Redundancy}
CompGCN's message-passing \emph{already captures} community structure from graph topology. In MetaFam, community $\equiv$ connected component. Two CompGCN layers produce naturally clustered representations without explicit community labels, making the one-hot encoding redundant.

\subsubsection{Forest-of-Trees Limitations}
...
\subsection{The Precision vs Recall Trade-off}

The asymmetric metric gap is the most revealing finding:

\begin{itemize}
    \item \textbf{Hits@10 parity} (0.968 vs 0.972): CompGCN identifies the correct entity within the top-10 almost as reliably as RotatE, demonstrating solid neighborhood-level understanding.
    \item \textbf{Hits@1 gap} (0.509 vs 0.676): The features introduce noise that blurs fine-grained distinctions. The community signal overwhelms subtle structural differences needed for precise head-of-list ranking.
\end{itemize}

This pattern suggests that custom features help with \textbf{coarse entity discrimination} but hurt \textbf{fine-grained ranking}---a classic noise-vs-signal trade-off.

\subsection{Positive Outcomes}

\begin{enumerate}
    \item \textbf{Best GNN model:} CompGCN-Custom (0.677 MRR) outperforms RGCN\_RotatE (0.442) by 53\%, validating that domain features + composition-based architecture $>$ random init + RGCN.
    \item \textbf{Cross-task integration:} First model combining Task 1 features, Task 2 communities, and Task 4 link prediction---demonstrating end-to-end knowledge transfer.
    \item \textbf{Architectural validation:} The hot-swap feature injection approach works seamlessly with PyKEEN, enabling rapid future experimentation.
    \item \textbf{Near-perfect recall:} Hits@10 of 0.968 confirms the model's fundamental entity discrimination capability.
\end{enumerate}
...
\subsection{Task 5: Feature-Enhanced CompGCN}

\begin{itemize}
    \item \textbf{CompGCN-Custom is the best GNN model:} 0.677 MRR, outperforming RGCN\_RotatE (0.442) by 53\%
    \item \textbf{Standard RotatE still wins:} 0.791 MRR vs 0.677 MRR ($-$11.4 pp), showing free embeddings outperform constrained feature projections on MetaFam
    \item \textbf{Near-equal Hits@10:} 0.968 vs 0.972 ($-$0.4 pp)---features help broad recall but hurt precision (Hits@1: 0.509 vs 0.676)
    \item \textbf{Community features are redundant:} CompGCN message passing already captures community $\equiv$ connected component structure
    \item \textbf{Cross-task integration validated:} First model combining Tasks 1, 2, and 4 outputs end-to-end
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[task, fill=blue!15, draw=blue] (t5) at (10,2) {Task 5\\CompGCN};
        ...
        \draw[arrow, blue, thick] (t1) -- (t5) node[midway, above, sloped] {\small features};
        \draw[arrow, blue, thick] (t2) -- (t5) node[midway, above, sloped] {\small communities};
        \draw[arrow, blue, thick] (t4) -- (t5) node[midway, below] {\small pipeline};
    \end{tikzpicture}
    \caption{Task Dependencies and Information Flow. Task 5 (blue) integrates outputs from Tasks 1, 2, and 4.}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Tasks 1+2 $\rightarrow$ Task 5:} Node features (gender, generation, centrality) and community IDs are injected as CompGCN initial representations
    \item \textbf{Task 4 $\rightarrow$ Task 5:} Link prediction framework (splits, evaluation, RotatE decoder) is reused and extended with GNN-based entity representation
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{ComplEx} is the best model for clean, synthetic family KGs
    \item Consider \textbf{GNN+decoder} when neighborhood structure is informative, but prefer \textbf{CompGCN over RGCN} for multi-relational graphs
    \item \textbf{Feature injection} helps GNN recall but can hurt precision---use learned embeddings rather than sparse one-hot features for community encoding
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Learned community embeddings:} Replace one-hot encoding with low-dimensional community embeddings to improve CompGCN feature quality
    \item \textbf{Feature ablation:} Systematically isolate which node features benefit GNN link prediction vs introduce noise
    \item \textbf{Connected KG evaluation:} Test CompGCN-Custom on denser graphs where cross-community message passing is possible
\end{enumerate}
